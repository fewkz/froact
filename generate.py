...
def filter_class_field(klass_name, field_name):
    klass = next(
        klass for klass in dump["Classes"] if klass_name == klass["Name"])
    member = next(
        (member for member in klass["Members"]
         if member["Name"] == field_name), None
    )
    return (
        "ReadOnly" not in member.get(
            "Tags", []) and parse_security(member["Security"])
        if member
        else False
    )
...
def class_props_type(klass_name):
    ...
    if INLINE_INHERITED_CALLBACKS:
        class_fields.extend(
            get_class_signal_fields_recursive(klass_name, klass_name))
        class_fields.extend(
            get_class_bind_fields_recursive(klass_name, klass_name))
    else:
...
# Binary search for finding broken types
b_search: list[Literal["left", "right"]] = []
index = 0
min_index = 0
